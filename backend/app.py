...
def compare_years():
    ...
    total_change = percent_change(stats1["total"], stats2["total"])
    ...
    for o in stats1["offenses"]:
        offense_map[o["offense_name"]] = [o["count"], 0]

    for o in stats2["offenses"]:
        if o["offense_name"] in offense_map:
            offense_map[o["offense_name"]][1] = o["count"]
        else:
            offense_map[o["offense_name"]] = [0, o["count"]]

    biggest_change = None
    max_diff = 0

    for offense, (c1, c2) in offense_map.items():
        diff = abs(c2 - c1)
        if diff > max_diff:
            max_diff = diff
            biggest_change = (offense, c1, c2, c2 - c1)
